...
@router.get("/id/{problem_id}", response_model=ProblemResponse)
async def get_problem_by_id_route(
    problem_id: str,
    current_user=Depends(get_current_user)
):
    """Get a single problem by MongoDB id."""
    user_id = str(current_user["_id"])
    problem = await get_problem_by_id(problem_id, user_id)
    if not problem:
        raise HTTPException(
...
@router.get("/{topic_or_id}", response_model=List[ProblemResponse])
async def get_problems_or_single(
    ...
    and legacy GET /problems/{problem_id} calls.

    Topics return a list. ObjectId paths return a one-item list.
    """
    user_id = str(current_user["_id"])
